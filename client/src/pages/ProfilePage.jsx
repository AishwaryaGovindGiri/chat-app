    ...
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
     ...
     border-gray-600 flex items-center justify-between max-sm:flex-col-reverse
      rounded-lg'>
        ...
        {/* in normal html the for = "avatar" */}
          <label htmlFor="avatar" className='flex items-center 
          gap-3 cursor-pointer'>
            {/* this htmlFor="avatar" is connected to input who has the id='avatar */}
            <input onChange = {(e)=>setSelectedImg(e.target.files[0])} type="file" id="avatar" accept='.png , .jpg , .jpeg' hidden />
            {/* if img selected then create URL  */}
            {/* 1.user select image
                2.image become file object(but img tag can't show file object
                3.so we use createobjecturl --> this create temp url for image so browser can show preview of img
                4. it is like temp address to show the image ) */}
